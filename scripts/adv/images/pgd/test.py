...
# datax = np.arange(3**4).reshape((-1,1))
# namess = np.array([['a','b','c'],['d','e','f'],['x','y','z'],['p','q','r']])
# midx = pd.MultiIndex.from_product(namess, names=['c1','c2','c3','c4'])
# df = pd.DataFrame(datax, index = midx, columns=['data'])
# df.to_csv('test.csv')
# print(df)
...
line_chart = figure(width=1000, height=400, x_axis_type="datetime",
...
scatter = figure(width=500, height=400,
